(function () {
  ...
  async function enterApplication() {
    ...
    setText("accountName", profile?.name || session.user?.email?.split("@")[0] || "Usuário");
    setText("accountAvatar", initials(profile?.name || session.user?.email));
    setText("accountRole", admin ? "Administrador" : "Conta ScannerDay");
    setText("rolePill", admin ? "ADMINISTRADOR" : "USUÁRIO");
    setText("dashboardEyebrow", admin ? "PAINEL ADMINISTRATIVO" : "ANÁLISES SCANNERDAY");
    setText("dashboardTitle", admin ? "Visão operacional." : "Suas oportunidades.");
    setText("dashboardSubtitle", admin ? "Acompanhe dados, integrações e análises publicadas." : "Veja as análises completas e as apostas sugeridas pela equipe.");
    setText("scanBtn", admin ? "⌁ Executar scanner" : "Ver análises");
