...
module.exports = async function handler(request, response) {
  try {
    ...
    const odd = Number(body.entry_odd), stake = 1, confidence = Number(body.confidence);
    if (!(odd > 1) || confidence < 0 || confidence > 100) return response.status(422).json({ error: "Odd ou confiança inválidos" });
