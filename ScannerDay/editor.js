(function () {
  ...
  byId("validateAnalysisFile").onclick = async () => {
    try {
      ...
      const result = await api("/api/editor-validate", { method: "POST", body: JSON.stringify({ file_name: selectedFile.name, payload: parsedPayload }) });
  ...
  byId("publishAnalyses").onclick = async () => {
    ...
    try {
      const result = await api("/api/editor-publish", { method: "POST", body: JSON.stringify({ file_name: selectedFile.name, payload: parsedPayload }) });
